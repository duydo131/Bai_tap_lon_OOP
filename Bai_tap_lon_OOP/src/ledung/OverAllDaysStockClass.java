...
public class OverAllDaysStockClass extends OverAllDays {
	
	private String name;
	
	// constructor
	
	public OverAllDaysStockClass(String name, Date date1, Date date2) {
		super(date1, date2);
		this.name = name;
	}
	...
	public int increaseCodeCounter() {
		...
		while (InputData.isWeekend(date1)) {
			date1 = InputData.getTomorrow(date1);
		}
		
		if (date1==date2) {
			OverAllDayStockClass p = new OverAllDayStockClass(date1, name);
			return p.increaseCounter(name);
		}
	...
	public int increaseCodeCounter(String name) {
		...
		if (name == "VN30") {
			for (STOCK i : InputData.stockVN30()) {
				closedValue = InputData.getTodayVN30(date2).get(i).getGiaDongCua();
				openValue = InputData.getTodayVN30(date1).get(i).getGiaMoCua();
				diff = closedValue - openValue;
				if (diff > 0) {
					counter++;
				}
			...
			for (STOCK i : InputData.stockHNX30()) {
				closedValue = InputData.getTodayHNX30(date2).get(i).getGiaDongCua();
				openValue = InputData.getTodayHNX30(date1).get(i).getGiaMoCua();
				diff = closedValue - openValue;
				if (diff > 0) {
					counter++;
				}
